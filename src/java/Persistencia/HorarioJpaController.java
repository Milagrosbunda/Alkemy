...
public class HorarioJpaController implements Serializable {
    ...
    public void create(Horario horario) {
        ...
        try {
            ...
            if (alumno != null) {
                alumno = em.getReference(alumno.getClass(), alumno.getLegajo());
                horario.setAlumno(alumno);
            }
            em.persist(horario);
            if (alumno != null) {
    ...
    public void edit(Horario horario) throws NonexistentEntityException, Exception {
        ...
        try {
            em = getEntityManager();
            em.getTransaction().begin();
            Horario persistentHorario = em.find(Horario.class, horario.getId_hora());
            Alumno alumnoOld = persistentHorario.getAlumno();
            Alumno alumnoNew = horario.getAlumno();
            if (alumnoNew != null) {
                alumnoNew = em.getReference(alumnoNew.getClass(), alumnoNew.getLegajo());
                horario.setAlumno(alumnoNew);
            }
            horario = em.merge(horario);
            if (alumnoOld != null && !alumnoOld.equals(alumnoNew)) {
                alumnoOld.getHorarios().remove(horario);
                alumnoOld = em.merge(alumnoOld);
            }
            if (alumnoNew != null && !alumnoNew.equals(alumnoOld)) {
                alumnoNew.getHorarios().add(horario);
    ...
    public void destroy(int id) throws NonexistentEntityException {
        ...
        try {
            ...
            Alumno alumno = horario.getAlumno();
            if (alumno != null) {
                alumno.getHorarios().remove(horario);
                alumno = em.merge(alumno);
            }
            em.remove(horario);
            em.getTransaction().commit();
